...
def data_functions():
    def load_data_from_excel(path="andy_data.xlsx"):
        ...
        df_core = enrich_data_by_doi(df_core, True)
...
def get_openalex_works_bulk_by_id(
    ids: list[str], id_type: str = "doi", per_page: int = 50
) -> pl.DataFrame:
...
@app.function
def enrich_data_by_doi(input_df:pl.DataFrame, store:bool = True) -> pl.DataFrame:
    """Enrich the input DataFrame with metadata fetched via DOIs."""
    input_df = input_df.with_columns(
        pl.col("DOI").str.strip_chars().str.to_lowercase().str.replace_all(",","").alias("DOI_clean")
    )
    dois = (
        input_df.select("DOI_clean")
        .filter(pl.col("DOI_clean").is_not_null())
        .unique()
        .to_series()
        .to_list()
    )
    ...
    if store:
        oa_works_df.write_parquet("oa_results.xlsx")
    # merge back to input_df
    input_df = input_df.join(oa_works_df, left_on="DOI_clean", right_on="doi", how="left", suffix="_oa")
...
def ui_controls(df_full):
    ...
    text_concept = mo.ui.text_area(
        value=default_concept, label="Target Concept", rows=2
    )
    slider_threshold = mo.ui.slider(
        start=0.3, stop=0.8, step=0.01, value=0.45, label="Relevance"
    )
    ...
    slider_cluster_min = mo.ui.slider(
        start=5, stop=100, step=5, value=20, label="Min Size"
    )
    ...
    slider_opacity = mo.ui.slider(
        start=0.1, stop=1.0, step=0.1, value=0.6, label="Dots opacity"
    )
    slider_size = mo.ui.slider(start=10, stop=200, value=60, label="Dot size")
    # add sliders for cloud opacity and spread
    slider_cloud_opacity = mo.ui.slider(
        start=0.01, stop=0.2, step=0.01, value=0.03, label="Cloud Opacity"
    )
    slider_cloud_spread = mo.ui.slider(
        start=1000, stop=10000, step=500, value=5000, label="Cloud Spread"
    )
    ...
    # Layout: 3 Columns
    controls_layout = mo.hstack([col_data, col_algo, col_viz], gap="lg", align="start")
...
def logic_calculation(
    ...
    if not check_recalc.value:
        ...
        if len(df_filtered) < 10:
            ...
            # Update topic_info_pd with the refined labels before converting to Polars
            ...
            _coord_df = df_filtered.select(["Title", "Label", "semantic_score", "keyword_match", "Type group"])
            ...
            # Apply labels via map (Polars doesn't support dict map directly on series efficiently, use join)
...
def visualization(
    ...
    if results is None:
        ...
        if check_selection.value:
            ...
            points = points_base.encode(
                ...
                tooltip=["Title", "topic_label", "semantic_score"],
        ...
        text_outline = (
            ...
            .encode(x="x", y="y", text="topic_label")
        )
        ...
        final_chart = (bg_layer + points + text_outline + text_fill).properties(
            width=800, height=600, title="Topic Clusters"
        )
    ...
    return chart, coord_df, final_chart, results


@app.cell
def _(final_chart):
    # export high res image of the chart
    final_chart.save("topic_clusters.png", scale_factor=3)
    return
...
@app.cell
def main_layout(chart, control_panel, result_view):
    mo.vstack([control_panel, mo.hstack([chart, result_view], widths=[7, 5])])
    return
